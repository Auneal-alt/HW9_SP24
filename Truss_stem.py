    ...
    def ImportFromFile(self, data):
        ...
        while i < len(data):
            ...
            if L.find('#') == 0:
                ...
                self.importTitle(data, i)
                pass
            elif L.find('link') >= 0:
                # read in a pipe and increase i to end of pipe section
                self.importLink(data, i)
                pass
            elif L.find('node') >= 0:
                self.importNode(data, i)
                pass
            elif L.find('material') >= 0:
                self.importMaterial(data, i)
                pass
            elif L.find('static_factor') >= 0:
                self.importSF(data, i)
                pass
            i += 1
...
class TrussView():
    ...
    def drawAGrid(self, DeltaX=10, DeltaY=10, Height=320, Width=180, CenterX=120, CenterY=60, Pen=None, Brush=None):
        ...
        """
        # JES MISSING CODE HERE$
        # To identify the lines using H for height, W for width, L for Left,... for Right, Top, and Bottom
        # After way over simplifying the code I couldnt get it to work I used chatgpt for help and
        # it changed it to have an if/else Statement
        ...
        # To create the background as a rectangle and make it so we can draw on it
    ...
    def drawNodes(self, truss=None, scene=None):
        ...
        for node in truss.nodes:
            ...
            self.drawACircle(x, y, 7, brush=BNFill, pen=PNOutline, name=('node: ' + node.name))
            self.drawALabel(x - 15, y + 15, str=node.name, pen=PNLabel)
        pass
